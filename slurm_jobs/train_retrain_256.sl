...
echo "============================================"
echo "Retrain with 256x256 patches: PT37 (10) + UMich (2)"
...
CONFIG="tutorials/fiber_retrain_256.yaml"
...
echo ""
echo "######################################################################"
echo "# STEP 1/3: Training (800 epochs, 4x A10 DDP, 256x256 patches)"
echo "######################################################################"
echo "Config: $CONFIG"
echo "Started: $(date)"
...
CKPT=$(find outputs/fiber_retrain_256/ -name "last.ckpt" -type f -printf '%T@ %p\n' 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-)

if [ -z "$CKPT" ]; then
    echo "❌ Could not find checkpoint in outputs/fiber_retrain_256/"
    exit 1
fi
